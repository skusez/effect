...
function generateCharacter(node: Character, length: number, state: Model.GenerationState): string | undefined {
  const preferred = preferredCodePoints.filter((codePoint) =>
    codePointLength(codePoint) === length && codePointInIntervals(codePoint, node.intervals)
  )
  ...
  let count = 0
  for (const interval of node.intervals) count += countCodePoints(interval, length)
...
function canonicalCharacter(node: Character, length: number): string | undefined {
  const preferred = preferredCodePoints.find((codePoint) =>
    codePointLength(codePoint) === length && codePointInIntervals(codePoint, node.intervals)
  )
...
function generateSequence(
  ...
  for (let index = 0; index < nodes.length; index++) {
    ...
    const tail = concatenation(nodes.slice(index + 1))
    const nodeLengths = possibleLengths(node, remaining, cache)
    const tailLengths = possibleLengths(tail, remaining, cache)
...
export function compile(pattern: Pattern): Compiled | undefined {
  ...
  return {
    ...
    shrink: (value, minimumLength) => structuralShrinks(node, regExp, value, minimumLength)
  }
}
